...
def get_latest_version(models_path):
    ...
    versions = [
        int(re.search(r"V(\d+)", file).group(1)) for file in files if re.search(r"V(\d+)", file)
    ]
...
if __name__ == "__main__":
    ...
    save_model = os.path.join(
        models_path,
        f"lenet-{{epoch:02d}}-{{accuracy:.3f}}-{{val_accuracy:.3f}}-V{latest_version + 1}.model",
    )
    ...
    train_ds = train_ds.map(
        lambda x, y: (rescaling_layer(x), y), num_parallel_calls=AUTOTUNE
    )
    val_ds = val_ds.map(
        lambda x, y: (rescaling_layer(x), y), num_parallel_calls=AUTOTUNE
    )
    ...
    results_df[
        "experiment_description"
    ] = f"V{latest_version + 1} Adicionar descricao"
